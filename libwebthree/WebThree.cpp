	...
	m_net(_clientVersion, _n, _network)
{
	...
	if (_interfaces.count("eth"))
	{
		...
		if (_params.sealEngineName == "Ethash")
			m_ethereum.reset(new eth::EthashClient(_params, (int)_params.u256Param("networkID"), &m_net, shared_ptr<GasPricer>(), _dbPath, _snapshotPath, _we));
			...
			m_ethereum.reset(new eth::Client(_params, (int)_params.u256Param("networkID"), &m_net, shared_ptr<GasPricer>(), _dbPath, _snapshotPath, _we));
